...
def seed_labs(apps, schema_editor):
    SkoolLab = apps.get_model("skool", "SkoolLab")
    for order, title, filename, description in LABS:
        SkoolLab.objects.update_or_create(
            title=title,
            defaults={
...
class Migration(migrations.Migration):
    dependencies = [("skool", "0005_skooluser_labs_welcome_seen")]

    operations = [migrations.RunPython(seed_labs, migrations.RunPython.noop)]
