...
class LandingTests(TestCase):
    ...
    def test_home(self):
        ...
        self.assertContains(response, 'Öğrenmekten öte; iş bulmanız veya kendi işinizi yapmanız için tasarlandı.')
        self.assertContains(response, 'Kağıt üstünde değil,')
        self.assertContains(response, '80+ saatlik bootcamp')
        self.assertContains(response, 'Risk temelli gerçek hikâyelerinizle ayrışır')
        self.assertContains(response, '12 ay boyunca sınırsız birebir görüşme ve çalışma desteği')
        ...
        self.assertContains(response, 'KÜRESEL DÜZENLEME DALGASI')
        self.assertContains(response, 'AVRUPA BİRLİĞİ • DORA')
        self.assertContains(response, 'AI ACT + NIS2')
        self.assertContains(response, 'ABD • SEC 2023')
        self.assertContains(response, 'Siber Güvenlik Başkanlığı kuruldu')
        self.assertEqual(response.content.decode().count('https://www.resmigazete.gov.tr/eskiler/2025/03/20250319-1.htm'), 1)
        self.assertNotContains(response, 'Bilgiden uygulamaya. Uygulamadan hikâyeye.')
        self.assertNotContains(response, '80 saatte kendi denetim hikâyelerini oluşturmaya başla.')
        ...
        self.assertContains(response, 'Gerçeğe Uygun Denetim Vakaları')
        self.assertContains(response, 'mülakata girdiğinde anlatacağın mesleki hikâyeye')
        self.assertContains(response, 'Gerçek kurumlarla herhangi bir bağlantısı veya ilgisi yoktur.')
        self.assertNotContains(response, 'Gerçek Denetim Vakaları')
